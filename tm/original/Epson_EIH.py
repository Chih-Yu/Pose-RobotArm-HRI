...
def EIHCali():
    ...
    objp[:, :2] = np.mgrid[0:w, 0:h].T.reshape(-1, 2)  # 在这里乘棋盘大小
    # print("OBJP", objp)
    # 储存棋盘格角点的世界坐标和图像坐标对
    ...
    for i in range(0, 15):
        ...
        img = cv2.imread("epson/img/" + 'color_' + str(i) + ".png")
    ...
    CaliPos = np.loadtxt('epson/img/Epson_CalibPos.txt', delimiter=',')
    ...
    for i in range(15):
        ...
        # tmpr = cv2.Rodrigues(np.array(rvecs[i]))[0]
        # print(tmpr)
        print(float(CaliPos[i, 0]), ',', float(CaliPos[i, 1]), ',',float(CaliPos[i, 2]), ',', float(CaliPos[i, 5]), ',', float(CaliPos[i, 4]), ',',
              float(CaliPos[i, 3]), ',')
