...
def rt2xyzuvw(rt):
    ...
    uvw = rotationMatrixToEulerAngles(r)

    return np.append(t,uvw)
...
def calM2C(img, mtx, id_order):
    ...
    return M2C, markerCorners, markerCenters.reshape(len(id_order),2)
...
# initial
# pos = np.array([[-325.612, 18.64, 219.342, -179.105, 0, 179.155],
#                 [-325.612, 18.64, 219.342, -179.074, 14.949, 179.395],
#                 [-325.612, 18.64, 219.342, -179.164, 0.321, 158.173]])

# id_order = np.array([4, 1, 3, 2])
# id_order = id_order.reshape(len(id_order), 1)

# img = cv2.imread('color_1.png')
# fs = cv2.FileStorage('Extrinsic0.txt', cv2.FILE_STORAGE_READ)
# mtx = fs.getNode('intrinsic').mat()
# C2G=fs.getNode('Camera2Gripper').mat()
...
# M2B_T=[]
# for i in range(4):
#     M2B_R_tmp, M2B_T_tmp=calM2B(pos[1][3:], pos[1][:3], C2G, M2C[i], depth=None)
...
def calM2B_R_4(M2B_T):
    M2B_t_vect = np.zeros((4, 3))

    M2B_t_vect[1] = M2B_T[1] - M2B_T[0]
    M2B_t_vect[2] = M2B_T[2] - M2B_T[0]
    crossvect0 = np.cross(M2B_t_vect[1], M2B_t_vect[2])
    norvect0 = crossvect0 / np.sqrt(np.sum(crossvect0 ** 2))

    M2B_t_vect[3] = M2B_T[3] - M2B_T[0]

    M2B_t3_2 = M2B_T[3] - M2B_T[2]
    M2B_t3_1 = M2B_T[3] - M2B_T[1]
    crossvect3 = np.cross(M2B_t3_2, M2B_t3_1)
    norvect3 = crossvect3 / np.sqrt(np.sum(crossvect3 ** 2))

    norvect = np.array([(norvect0 + norvect3) / 2])

    M2B_t_calrot = np.concatenate((M2B_t_vect[1:], norvect)).T
    M2F_t=np.array([[0,0,0],[60,0,0],[0,-60,0],[60,-60,0]])
    M2F_t_calrot = np.concatenate((M2F_t[1:], [[0,0,-1]])).T
...
# print(calM2B_R(M2B_T))


def calM2B_R_2(M2B_T):
    M2B_t_vect = np.zeros((3, 3))

    M2B_t_vect[0] = M2B_T[1] - M2B_T[0]
    M2B_t_vect[0][2] = 0
    M2B_t_vect[1] = np.array([0,0,-1])
